...
def build_shared_mlp(mlp_spec, act_first=False, use_layer_norm=False, act_fn=nn.GELU(approximate='tanh')):
...
def LIN_KERNEL(r: torch.Tensor, re):
    ...
    #    dis=r[i]
        #w[i]=(re / dis) - 1.0
    #    w[i]=((1-dis/re)**4)*(1+4.*dis/re)
    #print(w)    
    #w = ((1-r/re)**4)*(1+4.*r/re)
    #w=0.
    #hsml=0.5*re
    #q = r/hsml
    #factor = 15./(7.*3.1415926*hsml*hsml)
    #if torch.logical_or(q > 0., q <= 1.):
      
    #mask = torch.logical_and(q > 0., q <= 1.)  
    #w[mask] = factor * (2./3. - q[mask]*q[mask] + q[mask]**3 / 2.)
            
    #if torch.logical_or(q > 1., q <= 2):
    #mask = torch.logical_and(q > 1., q <= 2)
    #w[mask] = factor * 1./6. * (2.-q[mask])**3
        
    
    #mask = torch.logical_or(r > re, r < 0.0)
    mask = torch.logical_or(r > re, r < 1.0e-3)
    w[mask] = 0.
    #mask = torch.logical_and(r < 8.8e-3, r >= 1.0e-3)
    #w[mask] = 1.0
    return w
...
class SmoothGCN(nn.Module):
    def __init__(self,
                 ...
                 out_feat):
        super().__init__()
        self.mlp = build_shared_mlp([in_feat, out_feat], act_first=True)
        self.node_linear = nn.Linear(in_feat, in_feat)
        self.edge_linear = nn.Linear(in_feat, in_feat)
...
class NodeNetwork(nn.Module):
    def __init__(self,
        ...
        self.node_encoder = build_shared_mlp([in_feat, 32, 32])
        #self.node_decoder = build_shared_mlp([64, out_feat*8, out_feat])
        self.node_decoder = build_shared_mlp([32, 8, out_feat])
        self.cutoff = cutoff
        ...
        for l in range(layers_of_mp):
            ...
            self.mp_layers.append(nn.ModuleList([nn.LayerNorm(32), SmoothGCN(32, 32)]))
